...
public class Parser {
    ...
    /**
     * Synchronization for panic mode when the parser detects an error. Before the parser can get back to parsing, it
     * needs to get its state and the sequence of forthcoming tokens aligned such that the next token does match the
     * rule being parsed. This helps to balance between reporting as many distinct errors as there are, while minimising
     * cascaded (ghost) errors that don't indicate other real problems in the code.
     *
     * The natural point to synchronize is between statements. To do this, we jump out of any nested productions until
     * it gets back to that rule. Then, it synchronizes the token stream by discarding tokens until it reaches one that
